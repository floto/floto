			...
			switch (stepType) {
				...
				case "RUN":
					...
					if (flotoService.isUseProxy()) {
						line = "HTTP_PROXY='" + flotoService.getHttpProxyStaticNameUrl() + "' " + line;
					}
					...
					break;
				case "DETERMINE_IP":
					if ((hypervisorService instanceof WorkstationHypervisorService) &&
						!(hostManipulator instanceof SshHostManipulator) ) {
						String command = step.path("command").asText();
						File ipFile = null;
						try {
							String rand = RandomStringUtils.randomAlphanumeric(17).toUpperCase();
							ipFile = File.createTempFile("floto-", "-" + rand + ".guestFile");
							hypervisorService.runInVm(vmName, command + " > /ip." + rand);
							hypervisorService.runInVm(vmName, "chmod 755 /ip." + rand);
							hypervisorService.copyFileFromGuest(vmName, "/ip." + rand, ipFile);
							...
							hypervisorService.runInVm(vmName, "rm /ip." + rand);
						} catch (IOException e) {
						...
						flotoService.setExternalHostIp(host.name, host.ip);
						hostManipulator = new SshHostManipulator(host.ip);
					}
					break;
